...
function SubscriptionContent() {
  ...
  // Initialize step from URL params if present
  useEffect(() => {
    const step = searchParams.get("step");
    if (step === "form" || step === "completion") {
      setSubscriptionStep(step);
    }
  ...
  const handleCompleteSubscription = () => {
    ...
    window.history.replaceState(
      {},
      "",
      `/subscribe?step=completion&return=${encodeURIComponent(returnUrl)}`,
    );
  };
  ...
  const handlePlanSelect = (planId: string) => {
    ...
    window.history.replaceState(
      {},
      "",
      `/subscribe?step=form&plan=${planId}&return=${encodeURIComponent(returnUrl)}`,
    );
  };
  ...
  const handleBackToPlans = () => {
    ...
    window.history.replaceState(
      {},
      "",
      `/subscribe?return=${encodeURIComponent(returnUrl)}`,
    );
  };
